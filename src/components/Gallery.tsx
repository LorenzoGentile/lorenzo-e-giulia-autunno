...
const Gallery = () => {
  ...
  if (isLoading) {
    return <div className="section-container" id="galleria">
  ...
  return <div className="section-container" id="galleria">
      ...
      {photos.length === 0 ? <div className="text-center py-12 autumn-card">
          ...
          loop: true
              ...
              <CarouselContent>
                {photos.map(photo => <CarouselItem key={photo.id} className="md:basis-1/2 lg:basis-1/3">
                    <div className="group relative overflow-hidden rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 gallery-item timeline-item-animate photo-gallery-item h-full">
                      <div className="aspect-w-4 aspect-h-3 h-60">
                        <img src={photo.url} alt={photo.caption} className="object-cover w-full h-full transform group-hover:scale-105 transition-transform duration-300" loading="lazy" onError={e => {
